...
const CardDetailsPage = () => {
  const { searchContent, user } = useContext(AuthContext);
  ...
  const { data: singleNewsComment = [], refetch } = useQuery({
    ...
    queryFn: () =>
      fetch(`${process.env.REACT_APP_API_URL}comment/${params.id}`).then(
        (res) => res.json()
      ),
  });
    ...
    <div>
      {searchContent ? (
        ...
        <div className="flex w-full gap-0 lg:gap-5 h-full">
          ...
          <div>
            <h1 className="text-center text-xl font-semibold pb-5 hidden lg:block">
              Latest News
            </h1>
